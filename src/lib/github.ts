...
export async function getOctokitForInstallation(installationId: string | number) {
...
export async function getInstallationToken(installationId: string | number) {
  ...
  const { data } = await appOctokit.rest.apps.createInstallationAccessToken({
    installation_id: typeof installationId === "string" ? parseInt(installationId, 10) : installationId
  })
